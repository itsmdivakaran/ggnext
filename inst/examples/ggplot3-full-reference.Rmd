---
title: "ggplot3: every function, plot and option"
subtitle: "A worked reference for the whole package"
author: "Generated with ggplot3"
date: "`r Sys.Date()`"
output:
  html_document:
    toc: true
...
This document exercises **every exported function** of ggplot3 and **every
...
Writing a plot object in a chunk renders it inline — ggplot3 registers a
`knit_print()` method, so no `results='asis'` boilerplate is needed.

```{r}
packageVersion("ggplot3")
length(getNamespaceExports("ggplot3"))
```
...
```{r}
ggplot3(cars, aes(speed, dist, color = "steelblue")) + geom_point(size = 4)
```
...
```{r}
p_wide <- ggplot3(cars, aes(speed, dist), width = 900, height = 260) +
  geom_point()
p_wide
```
...
```{r}
b <- ggplot3:::build_geometry(ggplot3(cars, aes(speed, dist)) +
...
```{r}
ggplot3(cars, aes(speed, dist)) + geom_point(color = "#2B6BE0", size = 4,
                                             alpha = 0.7)
```
...
invisible(render(ggplot3(iris, aes(Species, Sepal.Width)) + geom_jitter()))
...
ggplot3(ts, aes(t, v, color = g)) + geom_line(linewidth = 2) + theme_minimal()
```
...
ggplot3(steps, aes(t, v)) + geom_step(color = "#12A594", linewidth = 2) +
...
```{r}
ggplot3(steps, aes(t, v)) + geom_line(dash = "6,4") + theme_minimal()
```
...
ggplot3(band, aes(t, ymin = lo, ymax = hi)) +
...
ggplot3(seg, aes(x, y, xend = xe, yend = ye)) +
...
ggplot3(db, aes(before, xend = after, y = g)) +
...
ggplot3(grid, aes(x, y, color = z)) + geom_tile() + theme_minimal()
```
...
ggplot3(cnt, aes(g)) + geom_bar() + theme_minimal()
```

```{r}
vals <- data.frame(g = c("alpha", "beta", "gamma", "delta"),
                   v = c(12, 27, 19, 8))
ggplot3(vals, aes(g, v, color = g)) + geom_col() +
  theme(legend_position = "none")
```
...
ggplot3(grp, aes(g, v, color = grp)) + geom_col(position = "stack") +
  labs(title = 'position = "stack" (default)') + theme_minimal()
```

```{r}
ggplot3(grp, aes(g, v, color = grp)) + geom_col(position = "dodge") +
  labs(title = 'position = "dodge"') + theme_minimal()
```

```{r}
ggplot3(grp, aes(g, v, color = grp)) + geom_col(position = "fill") +
...
```{r}
stacked <- ggplot3:::build_geometry(
  ggplot3(grp, aes(g, v, color = grp)) + geom_col(position = "stack")
)
...
```{r}
h <- plot_data(ggplot3(cars, aes(speed)) + geom_histogram(bins = 5))
h
...
```{r}
ggplot3(cars, aes(speed)) + geom_histogram(binwidth = 5) + theme_minimal()
```
...
```{r}
bx <- plot_data(ggplot3(iris, aes(Species, Sepal.Length)) + geom_boxplot())
...
ggplot3(eb, aes(g, m, ymin = lo, ymax = hi)) +
...
ggplot3(wf, aes(step, v)) + geom_waterfall() + theme_minimal()
```
...
ggplot3(lg, aes(x, y)) + geom_point(size = 5) + scale_x_log10() +
  theme_minimal()
```
...
```{r}
lb <- ggplot3:::build_geometry(
  ggplot3(lg, aes(x, y)) + geom_point() + scale_x_log10()
)
...
ggplot3(sq, aes(x, y)) + geom_point(size = 5) + scale_x_sqrt() +
  theme_minimal()
```

```{r}
ggplot3(sq, aes(x, y)) + geom_point(size = 5) + scale_y_reverse() +
...
ggplot3(srt, aes(name, value)) + geom_col() +
...
```{r}
ggplot3(vals, aes(g, v, color = g)) + geom_col() + coord_flip() +
  theme(legend_position = "none")
```
...
ggplot3(days, aes(d, v, color = d)) + geom_col() + coord_polar() +
  theme(legend_position = "none")
```
...
```{r}
ggplot3(days, aes(d, v, color = d)) + geom_col() +
...
```{r}
pol <- ggplot3:::build_geometry(
  ggplot3(days, aes(d, v)) + geom_col() + coord_polar()
)
...
```{r}
fixed <- ggplot3:::build_geometry(
  ggplot3(iris, aes(Sepal.Length, Sepal.Width)) + geom_point() +
    facet_wrap(Species))
free <- ggplot3:::build_geometry(
  ggplot3(iris, aes(Sepal.Length, Sepal.Width)) + geom_point() +
...
ggplot3(mt, aes(disp, mpg)) + geom_point() + facet_grid(am, cyl) +
  theme_minimal()
```
...
```{r}
lab_plot <- ggplot3(cars, aes(speed, dist)) + geom_point() +
  labs(title = "first") + labs(subtitle = "second") + xlab("X") + ylab("Y")
b <- ggplot3:::build_geometry(lab_plot)
...
theme_demo(theme_ggplot3(), "theme_ggplot3()")
```
...
```{r}
defaults <- ggplot3:::THEME_DEFAULTS
data.frame(
...
p0 <- ggplot3(cars, aes(speed, dist)) + geom_point()
data.frame(
...
ggplot3(radar, aes(axis, value, color = model)) +
...
ggplot3(tm, aes(size = revenue, label = region, color = region)) +
...
ggplot3(flows, aes(x = from, xend = to, y = n)) + geom_sankey()
```
...
net <- data.frame(from = c("A", "A", "B", "C", "D", "E", "B"),
                  to   = c("B", "C", "C", "D", "E", "A", "E"))
ggplot3(net, aes(x = from, xend = to)) + geom_network()
```
...
ch <- data.frame(from = c("A", "A", "B", "C"), to = c("B", "C", "C", "A"),
                 n = c(5, 3, 7, 2))
ggplot3(ch, aes(x = from, xend = to, y = n)) + geom_chord()
```
...
ggplot3(st, aes(t, v, color = grp)) + geom_stream() + theme_minimal()
```
...
ggplot3(bump, aes(year, rank, color = team)) +
...
ggplot3(fn, aes(stage, n, color = stage)) + geom_funnel() +
  theme(legend_position = "none")
```
...
ggplot3(par_d, aes(var, val, group = id, color = sp)) +
...
us <- data.frame(sets = c("A", "A&B", "B", "A&B&C", "C", "A&B",
                          "A", "B&C", "A&C", "A&B"))
ggplot3(us, aes(label = sets)) + geom_upset()
```
...
ggplot3(shap, aes(shap, feature, color = value)) +
...
ggplot3(pdp, aes(x, pred, group = id)) +
...
ggplot3(roc_d, aes(score = score, truth = truth)) +
...
ggplot3(cal, aes(pred, obs)) + geom_calibration() + theme_minimal()
```
...
ggplot3(lg_d, aes(score = score, truth = y)) +
...
ggplot3(cm, aes(predicted, actual)) + geom_confusion_matrix() +
  theme(legend_position = "none")
```
...
ggplot3(res, aes(fitted, resid)) + geom_residual() + theme_minimal()
```
...
ggplot3(lc, aes(n, score, color = split)) +
...
ggplot3(sil, aes(width, cluster, color = cluster)) +
...
ggplot3(emb, aes(d1, d2, color = cluster)) +
...
ggplot3(fc, aes(t, v, ymin = lo, ymax = hi, group = part)) +
...
ggplot3(km_d, aes(time = t, status = ev, color = arm)) +
...
ggplot3(ci, aes(time = t, status = ev)) + geom_cuminc() + theme_minimal()
```
...
ggplot3(fp, aes(hr, study, ymin = lo, ymax = hi, size = weight)) +
...
ggplot3(sw, aes(months, subject, color = response, label = ongoing)) +
...
ggplot3(sp, aes(month, pct, color = subject)) +
...
ggplot3(wr, aes(subject, pct)) + geom_waterfall_response() +
...
ggplot3(sg, aes(week, score, group = id)) + geom_spaghetti() + theme_minimal()
```
...
ggplot3(ba, aes(method_a, method_b)) + geom_bland_altman() + theme_minimal()
```
...
ggplot3(dr, aes(dose, resp)) + geom_dose_response() + scale_x_log10() +
  theme_minimal()
```
...
ggplot3(ae, aes(arm, ae, size = pct)) + geom_ae_heatmap() +
  theme(legend_position = "none")
```
...
ggplot3(sh, aes(baseline, followup)) + geom_shift() +
  theme(legend_position = "none")
```
...
ggplot3(cs, aes(label = stage, size = n)) + geom_consort()
```
...
```{r}
p_int <- ggplot3(iris, aes(Sepal.Length, Sepal.Width, color = Species)) +
  geom_point()
...
p_anim <- ggplot3(anim_d, aes(x, y)) + geom_point(size = 5) +
  animate(step, duration = 600)
ab <- ggplot3:::build_animation(p_anim, ggplot3:::build_geometry(p_anim))
...
```{r}
head(plot_data(ggplot3(cars, aes(speed, dist)) + geom_point()))
```

For a stat-backed layer it reflects what was computed, not the input:

```{r}
plot_data(ggplot3(iris, aes(Species, Sepal.Length)) + geom_boxplot())
```

Facets add a panel column:

```{r}
fd <- plot_data(ggplot3(iris, aes(Sepal.Length, Sepal.Width)) +
                  geom_point() + facet_wrap(Species))
head(fd)
table(fd$panel)
```
...
```{r}
multi <- ggplot3(cars, aes(speed, dist)) + geom_point() + geom_smooth()
...
write_plot_data(ggplot3(cars, aes(speed, dist)) + geom_point(), csv)
head(read.csv(csv), 3)
```
...
```{r results = "asis"}
cat(ggplot3_logo(width = 260))
```

```{r results = "asis"}
cat(ggplot3_logo(width = 260, style = "monogram"))
```
...
geom_cross <- function(mapping = NULL, data = NULL, ...) {
  ggplot3:::layer_new(GeomCross(), stat_identity(), mapping, data, list(...))
}

ggplot3(cars, aes(speed, dist)) + geom_cross() + theme_minimal()
```
...
show_error(render(ggplot3(mtcars, aes(disp, hp, color = class)) + geom_point()))
```

```{r}
# Missing required aesthetics.
show_error(render(ggplot3(cars, aes(speed)) + geom_point()))
show_error(render(ggplot3(cars, aes(speed, dist)) + geom_sankey()))
```

```{r}
# No layers, no data.
show_error(render(ggplot3(cars, aes(speed, dist))))
show_error(render(ggplot3() + geom_point()))
```

```{r}
# Impossible scales.
show_error(render(ggplot3(data.frame(x = c(0, 1, 10), y = 1:3), aes(x, y)) +
...
show_error(ggplot3(cars, aes(speed, dist)) + geom_point() + 42)
```

```{r}
# Faceting on a variable that does not exist.
show_error(render(ggplot3(iris, aes(Sepal.Length, Sepal.Width)) +
...
do.call(rbind, list(
  edge("single row",
       ggplot3(data.frame(x = 1, y = 1), aes(x, y)) + geom_point()),
  edge("two rows",
       ggplot3(data.frame(x = 1:2, y = 1:2), aes(x, y)) + geom_point()),
  edge("zero variance in x",
       ggplot3(data.frame(x = rep(5, 10), y = rnorm(10)), aes(x, y)) +
         geom_point()),
  edge("zero variance in both",
       ggplot3(data.frame(x = rep(1, 5), y = rep(2, 5)), aes(x, y)) +
         geom_point()),
  edge("NA in y",
       ggplot3(data.frame(x = 1:5, y = c(1, NA, 3, NA, 5)), aes(x, y)) +
         geom_point()),
  edge("very large values",
       ggplot3(data.frame(x = c(1e9, 2e9, 3e9), y = 1:3), aes(x, y)) +
         geom_point()),
  edge("very small values",
       ggplot3(data.frame(x = c(1e-9, 2e-9, 3e-9), y = 1:3), aes(x, y)) +
         geom_point()),
  ...
  edge("single facet level",
       ggplot3(iris[iris$Species == "setosa", ],
               aes(Sepal.Length, Sepal.Width)) +
         geom_point() + facet_wrap(Species)),
  edge("many categories (palette recycles)",
       ggplot3(data.frame(g = letters[1:12], v = 1:12),
