...
stage_values <- function(plot) {
  ...
  layer_values <- lapply(plot@layers, function(layer) {
    ...
    if (is.null(data)) {
      stop("Layer has no data: supply data to ggnext() or to the layer.")
    }
  ...
  if (is.null(facet)) {
    ...
    if (is.null(base_data)) {
      stop("Faceting requires plot-level data passed to ggnext().")
    }
