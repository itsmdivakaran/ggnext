...
#
# ggplot3 may not use the `scales` or `munsell` packages, so all color
...
GGPLOT3_DISCRETE_PALETTE <- c(
  "#E69F00", "#56B4E9", "#009E73", "#F0E442",
  "#0072B2", "#D55E00", "#CC79A7", "#999999"
)

# Endpoints for continuous color mapping (dark navy -> soft teal-green).
GGPLOT3_GRADIENT_LOW <- "#1A2E59"
GGPLOT3_GRADIENT_HIGH <- "#5FD0A5"
...
#'   (the legend key).
#' @noRd
map_color_discrete <- function(x, palette = GGPLOT3_DISCRETE_PALETTE) {
...
#' @noRd
map_color_continuous <- function(x, low = GGPLOT3_GRADIENT_LOW,
                                 high = GGPLOT3_GRADIENT_HIGH) {
