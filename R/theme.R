...
#' The default ggplot3 theme
#'
#' Light background with a softly tinted panel and white gridlines.
#'
#' @return A [Theme] to add to a plot with `+`.
#' @examples
#' p <- ggplot3(cars, aes(speed, dist)) + geom_point() + theme_dark()
#' @export
theme_ggplot3 <- function() make_theme()
...
#' [theme_ggplot3()]; pass `base` to restyle a different preset.
#'
#' @param ... Named [Theme] properties to override, e.g.
#'   `theme(panel_fill = "white", grid_color = "grey85")`.
#' @param base A [Theme] to start from; defaults to [theme_ggplot3()].
#' @return A [Theme] to add to a plot with `+`.
#' @examples
