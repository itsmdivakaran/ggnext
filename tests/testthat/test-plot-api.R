...
test_that("ggplot3() builds an empty plot specification", {
  p <- ggplot3(cars, aes(speed, dist))
  expect_true(S7::S7_inherits(p, Ggplot3Plot))
...
test_that("constructors validate their inputs", {
  expect_error(ggplot3(data = 1:10), "data frame")
  expect_error(ggplot3(cars, mapping = "aes"), "created with aes()")
})
...
test_that("interact() flags are carried into the buffer and the page", {
  p <- ggplot3(cars, aes(speed, dist)) + geom_point()
  buffer <- ggplot3:::build_geometry(p + interact(tooltip = TRUE, zoom = FALSE))
...
test_that("render output prints as a summary, not the raw document", {
  p <- ggplot3(cars, aes(speed, dist)) + geom_point()
  out <- render(p)
  expect_s3_class(out, "ggplot3_render")
  printed <- capture.output(print(out))
  expect_match(printed[1], "<ggplot3 static render:")
