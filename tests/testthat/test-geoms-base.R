...
test_that("geom_rect draws one rectangle per row from explicit corners", {
  ...
  m <- marks_of(ggplot3(d, aes(xmin = x1, xmax = x2, ymin = y1, ymax = y2)) +
                  geom_rect())
  expect_length(m, 2)
...
test_that("geom_polygon closes one shape per group", {
  d <- data.frame(x = c(1, 3, 2, 4, 6, 5), y = c(1, 1, 3, 1, 1, 3),
                  g = rep(c("a", "b"), each = 3))
  m <- marks_of(ggplot3(d, aes(x, y, color = g)) + geom_polygon())
  expect_length(m, 2)
...
test_that("geom_raster draws tiles without borders", {
  ...
  g$z <- seq_len(25)
  m <- marks_of(ggplot3(g, aes(x, y, color = z)) + geom_raster())
  expect_length(m, 25)
...
test_that("geom_abline spans the panel and does not train the scales", {
  ...
  plain <- panel_of(ggplot3(d, aes(a, b)) + geom_point())
  withab <- panel_of(ggplot3(d, aes(a, b)) + geom_point() + geom_abline(0, 1))
...
test_that("geom_abline respects intercept and slope", {
  b <- panel_of(ggplot3(cars, aes(speed, dist)) + geom_point() +
...
test_that("geom_blank draws nothing but still trains the scales", {
  b <- panel_of(ggplot3(cars, aes(speed, dist)) + geom_blank())
  expect_length(b$layers[[1]]$marks, 0)
  expect_equal(b$x$domain, ggplot3:::expand_domain(range(cars$speed)))
})

test_that("geom_label draws a plate behind every label", {
  d <- data.frame(x = c(1, 2), y = c(2, 1), l = c("alpha", "beta"))
  m <- marks_of(ggplot3(d, aes(x, y, label = l)) + geom_label())
...
test_that("geom_linerange and geom_crossbar draw their intervals", {
  ...
  lr <- marks_of(ggplot3(d, aes(g, ymin = lo, ymax = hi)) + geom_linerange())
  expect_length(lr, 2)
  expect_equal(unique(vapply(lr, function(k) k$type, character(1))), "line")

  cb <- marks_of(ggplot3(d, aes(g, m, ymin = lo, ymax = hi)) + geom_crossbar())
...
test_that("geom_errorbarh draws horizontal intervals with caps", {
  ...
  m <- marks_of(ggplot3(d, aes(y = g, xmin = lo, xmax = hi)) +
                  geom_errorbarh())
...
test_that("geom_curve bows away from the straight chord", {
  ...
  curved <- marks_of(ggplot3(d, aes(x, y, xend = xe, yend = ye)) +
                       geom_curve())[[1]]
  straight <- marks_of(ggplot3(d, aes(x, y, xend = xe, yend = ye)) +
                         geom_segment())[[1]]
...
test_that("geom_spoke turns angle and radius into endpoints", {
  ...
  v <- compute_stat(ggplot3:::StatSpoke(),
...
test_that("geom_count sizes points by how many share a position", {
  ...
  m <- marks_of(ggplot3(d, aes(x, y)) + geom_count())
...
test_that("geom_freqpoly bins like a histogram but draws a line", {
  m <- marks_of(ggplot3(cars, aes(speed)) + geom_freqpoly(bins = 8))
  expect_length(m, 1)
...
test_that("geom_function evaluates the function over its range", {
  m <- marks_of(ggplot3(data.frame(x = c(-3, 3)), aes(x)) +
...
test_that("geom_qq plots sample against theoretical quantiles", {
  ...
  vals <- compute_stat(ggplot3:::StatQQ(),
...
test_that("geom_qq_line passes through both quartiles", {
  set.seed(2)
  s <- stats::rnorm(200)
  vals <- compute_stat(ggplot3:::StatQQ(line = TRUE),
...
test_that("geom_dotplot stacks one dot per observation", {
  m <- marks_of(ggplot3(cars, aes(speed)) + geom_dotplot(binwidth = 2))
  expect_length(m, nrow(cars))
  # Dots in the same bin stack to successive heights starting at 1.
  d <- plot_data(ggplot3(cars, aes(speed)) + geom_dotplot(binwidth = 2))
...
test_that("quantile regression brackets the data and orders by tau", {
  ...
  vals <- compute_stat(ggplot3:::StatQuantile(quantiles = c(0.1, 0.5, 0.9)),
  ...
  rq_med <- ggplot3:::rq_fit(d$x, d$y, 0.5)
...
test_that("every new geom renders to both targets", {
  set.seed(1)
  plots <- list(
    rect = ggplot3(data.frame(x1 = 1, x2 = 2, y1 = 1, y2 = 2),
                   aes(xmin = x1, xmax = x2, ymin = y1, ymax = y2)) +
      geom_rect(),
    polygon = ggplot3(data.frame(x = c(1, 3, 2), y = c(1, 1, 3)), aes(x, y)) +
      geom_polygon(),
    abline = ggplot3(cars, aes(speed, dist)) + geom_point() + geom_abline(0, 3),
    blank = ggplot3(cars, aes(speed, dist)) + geom_blank(),
    label = ggplot3(data.frame(x = 1, y = 1, l = "hi"), aes(x, y, label = l)) +
      geom_label(),
    linerange = ggplot3(data.frame(g = "a", lo = 1, hi = 2),
                        aes(g, ymin = lo, ymax = hi)) + geom_linerange(),
    crossbar = ggplot3(data.frame(g = "a", m = 1.5, lo = 1, hi = 2),
                       aes(g, m, ymin = lo, ymax = hi)) + geom_crossbar(),
    errorbarh = ggplot3(data.frame(g = "a", lo = 1, hi = 2),
                        aes(y = g, xmin = lo, xmax = hi)) + geom_errorbarh(),
    rug = ggplot3(cars, aes(speed, dist)) + geom_point() + geom_rug(),
    curve = ggplot3(data.frame(x = 1, y = 1, xe = 3, ye = 3),
                    aes(x, y, xend = xe, yend = ye)) + geom_curve(),
    count = ggplot3(data.frame(x = c(1, 1, 2), y = c(1, 1, 2)), aes(x, y)) +
      geom_count(),
    freqpoly = ggplot3(cars, aes(speed)) + geom_freqpoly(bins = 6),
    fn = ggplot3(data.frame(x = c(-2, 2)), aes(x)) +
      geom_function(dnorm, xlim = c(-2, 2)),
    qq = ggplot3(data.frame(v = stats::rnorm(50)), aes(v)) +
      geom_qq() + geom_qq_line(),
    dotplot = ggplot3(cars, aes(speed)) + geom_dotplot(binwidth = 3),
    quantile = ggplot3(cars, aes(speed, dist)) + geom_quantile(),
    raster = ggplot3(local({
