...
test_that("interact(tooltip = columns) swaps in layer-data columns", {
  p <- ggplot3(iris, aes(Sepal.Length, Sepal.Width)) + geom_point() +
  ...
  expect_error(
    render(ggplot3(cars, aes(speed, dist)) + geom_point() +
...
test_that("a constant color inside aes() is honored literally", {
  p <- ggplot3(mtcars, aes(disp, hp, color = "blue")) + geom_point()
...
test_that("aes() names the problem when a mapping hits a function", {
  expect_error(
    ggplot3:::build_geometry(
      ggplot3(mtcars, aes(disp, hp, color = class)) + geom_point()
    ),
...
test_that("animate() builds one frame per level of the transition variable", {
  ...
  p <- ggplot3(d, aes(x, y)) + geom_point() + animate(step)
  ...
  expect_match(html, "ggplot3-scrub", fixed = TRUE)

  buffer <- ggplot3:::build_animation(p, ggplot3:::build_geometry(p))
...
test_that("animate() keeps axes fixed across frames", {
  ...
  p <- ggplot3(d, aes(x, y)) + geom_point() + animate(s)
  buffer <- ggplot3:::build_animation(p, ggplot3:::build_geometry(p))
