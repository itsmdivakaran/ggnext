...
test_that("normalize() maps a domain linearly onto [0, 1]", {
  expect_equal(
    ggplot3:::normalize(c(0, 5, 10), c(0, 10)),
    c(0, 0.5, 1)
  )
  expect_equal(
    ggplot3:::normalize(c(-2, 0, 2), c(-2, 2)),
    c(0, 0.5, 1)
  )
  # Values outside the domain map outside [0, 1] (clipping is the
  # renderer's job, not the scale's).
  expect_equal(ggplot3:::normalize(15, c(0, 10)), 1.5)
  expect_equal(ggplot3:::normalize(-5, c(0, 10)), -0.5)
})

test_that("normalize() refuses a degenerate domain", {
  expect_error(ggplot3:::normalize(1, c(3, 3)), "non-degenerate")
})

test_that("expand_domain() pads by 5% of the span on each side", {
  expect_equal(ggplot3:::expand_domain(c(0, 10)), c(-0.5, 10.5))
  expect_equal(ggplot3:::expand_domain(c(-10, 10)), c(-11, 11))
})
...
test_that("expand_domain() handles zero-span domains", {
  ...
  expect_equal(ggplot3:::expand_domain(c(5, 5)), c(4, 6))
  expect_equal(ggplot3:::expand_domain(c(0, 0)), c(-1, 1))
  expect_equal(ggplot3:::expand_domain(c(1000, 1000)), c(950, 1050))
})

test_that("nice_ticks() produces uniform, step-aligned, nice-valued ticks", {
  ticks <- ggplot3:::nice_ticks(4, 25, n = 5)
  expect_equal(ticks, seq(0, 25, by = 5))

  ticks <- ggplot3:::nice_ticks(0, 1, n = 5)
  ...
  for (rng in list(c(0.001, 0.017), c(-3, 141), c(2.2, 2.9), c(-1e6, 1e6))) {
    ticks <- ggplot3:::nice_ticks(rng[1], rng[2], n = 5)
    steps <- diff(ticks)
...
test_that("nice_ticks() snaps away floating point noise", {
  ticks <- ggplot3:::nice_ticks(0, 0.9, n = 5)
...
test_that("nice_ticks() handles a degenerate range", {
  expect_equal(ggplot3:::nice_ticks(7, 7), 7)
})
