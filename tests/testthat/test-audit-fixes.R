...
test_that("coord_flip() swaps axis titles, ticks and labels", {
  ...
  flipped <- panel_of(ggplot3(d, aes(g, v)) + geom_col() + coord_flip())
  plain <- panel_of(ggplot3(d, aes(g, v)) + geom_col())
...
test_that("flipped bars vary along x and sit in equal category slots", {
  ...
  m <- marks_of(ggplot3(d, aes(g, v)) + geom_col() + coord_flip())
...
test_that("geoms whose stat builds the marks still honour layer params", {
  ...
  ribbon_alpha <- function(a) {
    m <- marks_of(ggplot3(sk, aes(x = from, xend = to, y = n)) +
                    geom_sankey(alpha = a))
  ...
  labelled <- marks_of(ggplot3(sk, aes(x = from, xend = to, y = n)) +
                         geom_sankey(label = TRUE))
  bare <- marks_of(ggplot3(sk, aes(x = from, xend = to, y = n)) +
  ...
  node_r <- function(s) {
    m <- marks_of(ggplot3(net, aes(x = f, xend = t)) +
  ...
  fills <- marks_of(ggplot3(cs, aes(label = stage, size = n)) +
...
test_that("non-finite positions are dropped with a warning, not drawn", {
  ...
  expect_warning(
    svg <- render(ggplot3(d, aes(x, y)) + geom_point()),
...
test_that("Inf is dropped rather than crashing the tick algorithm", {
  ...
  expect_warning(svg <- render(ggplot3(d, aes(x, y)) + geom_point()),
                 "non-finite")
...
test_that("interval columns may still legitimately hold NA", {
  ...
  expect_silent(
    svg <- render(ggplot3(d, aes(t, v, ymin = lo, ymax = hi, group = part)) +
                    geom_forecast_band())
  )
  expect_match(svg, "^<svg ")
})
...
test_that("density geoms name the geom and the group when a group is too small", {
  ...
  expect_error(render(ggplot3(d, aes(y, color = g)) + geom_density()),
               "geom_density\\(\\).*group `b`.*at least 2")
  expect_error(render(ggplot3(d, aes(g, y)) + geom_violin()),
               "geom_violin\\(\\).*at least 2")
  expect_error(render(ggplot3(d, aes(y, g)) + geom_ridgeline()),
               "geom_ridgeline\\(\\).*at least 2")
})

test_that("geom_smooth refuses a fit it cannot compute", {
  expect_error(
    render(ggplot3(data.frame(x = 1:2, y = c(2, 4)), aes(x, y)) +
  ...
  svg <- render(ggplot3(cars, aes(speed, dist)) + geom_smooth(method = "lm"))
  expect_no_match(svg, "NaN")
})
...
test_that("bins = n yields exactly n bins, including empty ones", {
  ...
  expect_equal(nrow(plot_data(ggplot3(sparse, aes(z)) +
                                geom_histogram(bins = 10))), 10)
  set.seed(1)
  wide <- data.frame(z = stats::rnorm(500))
  for (b in c(5, 10, 30)) {
    d <- plot_data(ggplot3(wide, aes(z)) + geom_histogram(bins = b))
...
test_that("SVG attribute values are escaped", {
  ...
  quoted <- render(ggplot3(cars, aes(speed, dist)) + geom_point() +
                     theme(font = 'My "Font", sans'))
  expect_s3_class(xml2::read_xml(quoted), "xml_document")

  apos <- render(ggplot3(cars, aes(speed, dist)) + geom_point() +
                   theme(font = "Foo's Sans"))
  expect_s3_class(xml2::read_xml(apos), "xml_document")

  cat_quote <- render(ggplot3(data.frame(g = c('a"b', "c"), v = 1:2),
                              aes(g, v)) + geom_col())
...
test_that("the JSON writer emits valid JSON for every numeric edge case", {
  expect_equal(ggplot3:::to_json(Inf), "null")
  expect_equal(ggplot3:::to_json(-Inf), "null")
  expect_equal(ggplot3:::to_json(NaN), "null")
  # A tiny magnitude keeps scientific notation instead of 300 characters.
  expect_lt(nchar(ggplot3:::to_json(1e-300)), 30)
  # C0 control characters become \uXXXX escapes.
  expect_equal(ggplot3:::to_json("a\001b"), "\"a\\u0001b\"")
})
...
test_that("geom_spider_response(thresholds =) draws the RECIST lines", {
  d <- data.frame(m = rep(c(0, 2, 4), 2), p = c(0, -20, -35, 0, 10, 25),
                  s = rep(c("A", "B"), each = 3))
  with_thresh <- marks_of(ggplot3(d, aes(m, p, color = s)) +
                            geom_spider_response(thresholds = TRUE))
  without <- marks_of(ggplot3(d, aes(m, p, color = s)) +
...
test_that("geom_smooth(alpha =) sets the confidence band opacity", {
  band_alpha <- function(a) {
    marks_of(ggplot3(cars, aes(speed, dist)) +
...
test_that("gradient endpoints can be set independently", {
  p <- ggplot3(cars, aes(speed, dist, color = dist)) + geom_point()
...
test_that("a stat may supply an aesthetic the geom requires", {
  ...
  expect_match(render(ggplot3(d, aes(z)) + geom_point(stat = stat_bin(bins = 8))),
               "^<svg ")
  expect_match(
    render(ggplot3(data.frame(g = c("a", "a", "b")), aes(g)) +
             geom_point(stat = stat_count())),
    "^<svg "
  )
  # A genuinely missing aesthetic is still caught, with advice.
  expect_error(render(ggplot3(cars, aes(speed)) + geom_point()),
