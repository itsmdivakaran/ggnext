...
test_that("line geoms emit one polyline per group", {
  ...
  m <- marks_of(ggplot3(d, aes(x, y, color = g)) + geom_line())
  expect_length(m, 2)
...
test_that("geom_step interleaves horizontal-then-vertical corners", {
  ...
  m <- marks_of(ggplot3(d, aes(x, y)) + geom_step())
  expect_length(m, 1)
...
test_that("geom_area closes its polygon on the zero baseline", {
  ...
  buffer <- ggplot3:::build_geometry(ggplot3(d, aes(x, y)) + geom_area())
  ...
  norm0 <- ggplot3:::normalize(0, buffer$panels[[1]]$y$domain)
...
test_that("geom_segment and geom_dumbbell draw per-row primitives", {
  ...
  m <- marks_of(ggplot3(d, aes(a, b, xend = c, yend = d)) + geom_segment())
  expect_equal(mark_types(m), rep("line", 3))

  dd <- data.frame(lo = c(1, 2), hi = c(3, 5), g = c("u", "v"))
  m2 <- marks_of(ggplot3(dd, aes(lo, xend = hi, y = g)) + geom_dumbbell())
...
test_that("reference lines span the panel and ignore plot aes", {
  p <- ggplot3(cars, aes(speed, dist)) + geom_point() + geom_hline(50)
...
test_that("stat_count counts and stacked bars accumulate", {
  ...
  p <- ggplot3(d, aes(g, color = grp)) + geom_bar()
...
test_that("position = 'dodge' narrows bars and keeps them side by side", {
  ...
  p <- ggplot3(d, aes(g, color = grp)) + geom_bar(position = "dodge")
  m <- marks_of(p)
  expect_length(m, 4)
...
test_that("geom_boxplot renders box, whiskers, median, and outliers", {
  ...
  m <- marks_of(ggplot3(d, aes(g, y)) + geom_boxplot())
...
test_that("geom_violin emits one mirrored polygon per slot", {
  m <- marks_of(ggplot3(iris, aes(Species, Sepal.Width)) + geom_violin())
...
test_that("geom_waterfall draws bars plus connectors", {
  ...
  m <- marks_of(ggplot3(wf, aes(step, delta)) + geom_waterfall())
  tab <- table(mark_types(m))
...
test_that("discrete x scales train levels and label ticks with them", {
  ...
  buffer <- ggplot3:::build_geometry(ggplot3(d, aes(g, v)) + geom_point())
  ...
  buffer2 <- ggplot3:::build_geometry(ggplot3(d, aes(g, v)) + geom_point())
...
test_that("a discrete color mapping produces a legend in the buffer and SVG", {
  ...
  b2 <- ggplot3:::build_geometry(ggplot3(cars, aes(speed, dist)) + geom_point())
